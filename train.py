...
hyp = {'giou': 1.008,  # giou loss gain
       ...
       'iou_t': 0.3,  # iou target-anchor training threshold
...
def train(
    ...
    if resume:  # Load previously saved model
        if transfer:  # Transfer learning
            chkpt = torch.load(weights + 'best.pt', map_location=device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        #tanh iou_thres https://github.com/ultralytics/yolov3/issues/342
        model.hyp['iout'] = 0.3-0.1*np.tanh(1e-4*epoch**2)**2
        # Report time
